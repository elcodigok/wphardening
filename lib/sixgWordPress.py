...
class sixgWordPress():
    """
    This class creates and modifies the .htaccess file.
    ...
    def __init__(self, directory, verbose=False):
        ...
        self.htaccess = [
            ...
            '\tRedirectMatch 403',
            ' (?i)(&pws=0|_vti_|\(null\)|\{\$itemURL\}|echo(.*)',
            'kae|etc/passwd|eval\(|self/environ)', '\n',
            '\tRedirectMatch 403',
            ' (?i)\.(aspx?|bash|bak?|cfg|cgi|dll|exe|git|hg|ini|jsp|',
            'log|mdb|out|sql|svn|swp|tar|rar|rdf)$', '\n',
            '\tRedirectMatch 403 (?i)/(^$|(wp-)?config|mobiquo|phpinfo|shell|',
            'sqlpatch|thumb|thumb_editor|thumbopen|timthumb|webshell)\.php',
            '\n', '</IfModule>', '\n',
            '\n', '# 6G:[USER AGENTS]', '\n',
            '<IfModule mod_setenvif.c>', '\n',
            '\tSetEnvIfNoCase User-Agent ([a-z0-9]{2000,}) bad_bot', '\n',
            '\tSetEnvIfNoCase User-Agent (archive.org|binlar|casper|',
            'checkpriv|choppy|clshttp|cmsworld|diavol|dotbot|extract|',
            'feedfinder|flicky|g00g1e|harvest|heritrix|httrack|kmccrew|',
            'loader|miner|nikto|nutch|planetwork|postrank|purebot|pycurl|',
            'python|seekerspider|siclab|skygrid|sqlmap|sucker|turnit|',
            'vikspider|winhttp|xxxyy|youda|zmeu|zune) bad_bot', '\n',
            '\t# Apache < 2.3', '\n',
